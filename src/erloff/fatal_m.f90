module erloff_fatal_m
    ...
    contains
        ...
        procedure, public :: with_content_appended_s_
...
contains
    ...
    function with_content_appended_s(self, string) result(new_message)
        ...
        new_message = self%with_content_appended_s_(string)
    end function

    function with_content_appended_s_(self, string) result(new_error)
        class(fatal_t), intent(in) :: self
        type(varying_string), intent(in) :: string
        class(error_t), allocatable :: new_error
